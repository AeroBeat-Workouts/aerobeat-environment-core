...
func _init(kind: String = "") -> void:
	supported_kind = AeroEnvironmentConstantsScript.normalize_kind(kind)

func get_handler_name() -> String:
	return supported_kind

func supports_kind(kind: String) -> bool:
	return not supported_kind.is_empty() and AeroEnvironmentConstantsScript.normalize_kind(kind) == supported_kind
